...
async function seed() {
    ...
    const roles = [
        ...
        { name: "Employee", slug: "employee", permissions: ["dashboard:read", "project:read", "task:read", "task:update", "leave:create", "leave:read", "ticket:create", "ticket:read", "notification:read"] },
        { name: "Intern", slug: "intern", permissions: ["dashboard:read", "project:read", "task:read", "task:update", "leave:create", "ticket:create", "notification:read"] }
    ];
    for (const role of roles) {
        await Role.updateOne({ slug: role.slug }, role, { upsert: true });
    }
    const superRole = await Role.findOne({ slug: "super_admin" });
    await User.updateOne({ email: "superadmin@cybernaut.com" }, {
        firstName: "Cybernaut",
