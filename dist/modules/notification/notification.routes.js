...
const router = Router();
router.get("/", requireAuth, requirePermission("notification:read"), asyncHandler(async (req, res) => {
    const items = await Notification.find({ userId: req.user?.id }).sort({ createdAt: -1 }).limit(50).lean();
    res.json({ success: true, data: items });
}));
router.patch("/:id/read", requireAuth, requirePermission("notification:read"), asyncHandler(async (req, res) => {
