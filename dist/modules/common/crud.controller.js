...
export function createCrudController(model, resource) {
    ...
    const update = asyncHandler(async (req, res) => {
        const item = await model.findOneAndUpdate({ _id: req.params.id, isDeleted: false }, { ...req.body, updatedBy: req.user?.id }, { new: true, runValidators: true });
        if (!item)
            throw new AppError(404, `${resource} not found`, "NOT_FOUND");
    ...
    const remove = asyncHandler(async (req, res) => {
        const item = await model.findOneAndUpdate({ _id: req.params.id, isDeleted: false }, { isDeleted: true, updatedBy: req.user?.id }, { new: true });
        if (!item)
            throw new AppError(404, `${resource} not found`, "NOT_FOUND");
